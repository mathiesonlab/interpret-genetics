...
from msms_keras.MSMS_Generator import MSprime_Generator, Schrider_Generator
import utils
...
def main():
    ...
    cnn_model = load_model('models/schrider_model.hdf5', custom_objects={"rmse": rmse})
    ...
    msms_gen = Schrider_Generator(10, 1000000, 8000, 1000, 10000, yield_summary_stats=2)
    gen = msms_gen.data_generator(1)
    [X, distances], matrix, y_real = next(gen)
    #y_real = np.mean(y_real, axis=0)

    cnn_pred = cnn_model.predict([X, distances], batch_size=1)
